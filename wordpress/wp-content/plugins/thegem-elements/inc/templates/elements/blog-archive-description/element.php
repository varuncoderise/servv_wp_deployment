...
class TheGem_Template_Element_Blog_Archive_Description extends TheGem_Blog_Archive_Template_Element {
	
	public function __construct() {
	}
	
	public function get_name() {
		return 'thegem_te_blog_archive_description';
	}
	...
	public function shortcode_output($atts, $content = '') {
		...
		), $atts, 'thegem_te_blog_archive_description');
        ...
        <div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?>
             class="thegem-te-blog-archive-description <?= esc_attr($params['element_class']); ?> <?= esc_attr($uniqid); ?>">
		...
		<?php
		//Custom Styles
		$customize = '.thegem-te-blog-archive-description.'.$uniqid;
		$custom_css = '';
		...
		if (!empty($params['max_width'])) {
			$custom_css .= $customize.' .blog-description {max-width: ' . $params['max_width'] . 'px;}';
		}
		
		// Text Styles
		if ($params['text_letter_spacing'] != '') {
			$custom_css .= $customize.' .blog-description {letter-spacing: ' . $params['text_letter_spacing'] . 'px;}';
		}
		if ($params['text_transform'] != '') {
			$custom_css .= $customize.' .blog-description {text-transform: ' . $params['text_transform'] . ';}';
		}
		if (!empty($params['text_color'])) {
			$custom_css .= $customize.' .blog-description {color: ' . $params['text_color'] . ';}';
		}
		
		// Custom Font Styles
		if (!empty($params['use_custom_font_size'])) {
			if (!empty($params['custom_font_size'])) {
				$custom_css .= $customize.' .blog-description {font-size: '.esc_attr($params['custom_font_size']).'px;}';
			}
			if (!empty($params['custom_line_height'])) {
				$custom_css .= $customize.' .blog-description {line-height: '.esc_attr($params['custom_line_height']).'px;}';
			}
			if(!empty($params['custom_letter_spacing']) || strcmp($params['custom_letter_spacing'], '0') === 0) {
				$custom_css .= $customize.' .blog-description {letter-spacing: '.esc_attr($params['custom_letter_spacing']).'px;}';
			}
			if (!empty($params['custom_text_transform'])) {
				$custom_css .= $customize.' .blog-description {text-transform: '.esc_attr($params['custom_text_transform']).';}';
			}
		}
		if (!empty($params['use_custom_responsive_font'])) {
			if (!empty($params['custom_font_size_tablet'])) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .blog-description {font-size: '.esc_attr($params['custom_font_size_tablet']).'px;
				}}';
			}
			if (!empty($params['custom_line_height_tablet'])) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .blog-description {line-height: '.esc_attr($params['custom_line_height_tablet']).'px;
				}}';
			}
			if(!empty($params['custom_letter_spacing_tablet']) || strcmp($params['custom_letter_spacing_tablet'], '0') === 0 ) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .blog-description {letter-spacing: '.esc_attr($params['custom_letter_spacing_tablet']).'px;
				}}';
			}
			if (!empty($params['custom_font_size_mobile'])) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .blog-description {font-size: '.esc_attr($params['custom_font_size_mobile']).'px;
				}}';
			}
			if (!empty($params['custom_line_height_mobile'])) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .blog-description {line-height: '.esc_attr($params['custom_line_height_mobile']).'px;
				}}';
			}
			if(!empty($params['custom_letter_spacing_mobile']) || strcmp($params['custom_letter_spacing_mobile'], '0') === 0) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .blog-description {letter-spacing: '.esc_attr($params['custom_letter_spacing_mobile']).'px;
				}}';
			}
		}
		if (!empty($params['use_custom_font_size']) && !empty($params['use_custom_google_font'])) {
			...
			$custom_css .= $customize.' .blog-description .light, '.$customize.' .blog-description {'.esc_attr($font).'}';
		}
	...
	public function shortcode_settings() {
		...
		return array(
			'name' => __('Archive Blog Description', 'thegem'),
			'base' => 'thegem_te_blog_archive_description',
			'icon' => 'thegem-icon-wpb-ui-element-blog-description',
			'category' => __('Archive Blog Builder', 'thegem'),
			'description' => __('Archive Description (Archive Blog Builder)', 'thegem'),
			'params' => array_merge(
...
$templates_elements['thegem_te_blog_archive_description'] = new TheGem_Template_Element_Blog_Archive_Description();
$templates_elements['thegem_te_blog_archive_description']->init();

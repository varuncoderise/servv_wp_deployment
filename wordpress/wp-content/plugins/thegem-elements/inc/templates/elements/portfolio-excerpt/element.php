...
class TheGem_Template_Element_Portfolio_Excerpt extends TheGem_Portfolio_Item_Template_Element {

	public function __construct() {
	}

	public function get_name() {
		return 'thegem_te_portfolio_excerpt';
	}
	...
	public function shortcode_output($atts, $content = '') {
		...
		), $atts, 'thegem_te_portfolio_excerpt');
        ...
        <div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?> class="thegem-te-portfolio-excerpt <?= esc_attr($params['element_class']); ?> <?= esc_attr($uniqid); ?>">
            <<?= $params['text_tag'] ?> class="portfolio-excerpt <?= $params['text_styled'] ?>">
		...
		<?php
		//Custom Styles
		$customize = '.thegem-te-portfolio-excerpt.'.$uniqid;
		$custom_css = '';
		...
		if (!empty($params['max_width'])) {
			$custom_css .= $customize.' .portfolio-excerpt {max-width: ' . $params['max_width'] . 'px;}';
		}

		// Text Styles
		if ($params['text_letter_spacing'] != '') {
			$custom_css .= $customize.' .portfolio-excerpt {letter-spacing: ' . $params['text_letter_spacing'] . 'px;}';
		}
		if ($params['text_transform'] != '') {
			$custom_css .= $customize.' .portfolio-excerpt {text-transform: ' . $params['text_transform'] . ';}';
		}
		if (!empty($params['text_color'])) {
			$custom_css .= $customize.' .portfolio-excerpt {color: ' . $params['text_color'] . ';}';
		}

		// Custom Font Styles
		if (!empty($params['use_custom_font_size'])) {
			if (!empty($params['custom_font_size'])) {
				$custom_css .= $customize.' .portfolio-excerpt {font-size: '.esc_attr($params['custom_font_size']).'px;}';
			}
			if (!empty($params['custom_line_height'])) {
				$custom_css .= $customize.' .portfolio-excerpt {line-height: '.esc_attr($params['custom_line_height']).'px;}';
			}
			if(!empty($params['custom_letter_spacing']) || strcmp($params['custom_letter_spacing'], '0') === 0) {
				$custom_css .= $customize.' .portfolio-excerpt {letter-spacing: '.esc_attr($params['custom_letter_spacing']).'px;}';
			}
			if (!empty($params['custom_text_transform'])) {
				$custom_css .= $customize.' .portfolio-excerpt {text-transform: '.esc_attr($params['custom_text_transform']).';}';
			}
		}
		if (!empty($params['use_custom_responsive_font'])) {
			if (!empty($params['custom_font_size_tablet'])) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .portfolio-excerpt {font-size: '.esc_attr($params['custom_font_size_tablet']).'px;
				}}';
			}
			if (!empty($params['custom_line_height_tablet'])) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .portfolio-excerpt {line-height: '.esc_attr($params['custom_line_height_tablet']).'px;
				}}';
			}
			if(!empty($params['custom_letter_spacing_tablet']) || strcmp($params['custom_letter_spacing_tablet'], '0') === 0 ) {
				$custom_css .= '@media screen and (max-width: 1023px) and (min-width: 768px) {
				    '.$customize.' .portfolio-excerpt {letter-spacing: '.esc_attr($params['custom_letter_spacing_tablet']).'px;
				}}';
			}
			if (!empty($params['custom_font_size_mobile'])) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .portfolio-excerpt {font-size: '.esc_attr($params['custom_font_size_mobile']).'px;
				}}';
			}
			if (!empty($params['custom_line_height_mobile'])) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .portfolio-excerpt {line-height: '.esc_attr($params['custom_line_height_mobile']).'px;
				}}';
			}
			if(!empty($params['custom_letter_spacing_mobile']) || strcmp($params['custom_letter_spacing_mobile'], '0') === 0) {
				$custom_css .= '@media screen and (max-width: 767px) {
				    '.$customize.' .portfolio-excerpt {letter-spacing: '.esc_attr($params['custom_letter_spacing_mobile']).'px;
				}}';
			}
		}
		if (!empty($params['use_custom_font_size']) && !empty($params['use_custom_google_font'])) {
			...
			$custom_css .= $customize.' .portfolio-excerpt .light, '.$customize.' .portfolio-excerpt {'.esc_attr($font).'}';
		}
	...
	public function shortcode_settings() {
		...
		return array(
			'name' => __('Excerpt/Short Description', 'thegem'),
			'base' => 'thegem_te_portfolio_excerpt',
			'icon' => 'thegem-icon-wpb-ui-element-portfolio-excerpt',
			'category' => __('Portfolio Page Builder', 'thegem'),
			'description' => __('Excerpt/Short Description (Portfolio Page Builder)', 'thegem'),
			'params' => array_merge(
...
$templates_elements['thegem_te_portfolio_excerpt'] = new TheGem_Template_Element_Portfolio_Excerpt();
$templates_elements['thegem_te_portfolio_excerpt']->init();

...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			if ( true !== $this->is_tgmpa_complete() || true) {
				// Sort the plugins.
		...
		protected function add_admin_menu( array $args ) {
			$this->page_hook = add_submenu_page( $args['parent_slug'], $args['page_title'], $args['menu_title'], $args['capability'], $args['menu_slug'], $args['function'], $args['position'] );
		}
		...
		public function install_plugins_page() {
			...
			if (isset($thegemThemeOptions)) {
				$thegemThemeOptions->prePageWrapper('install-required-plugins');
			}
			...
			if ( ( ( 'tgmpa-bulk-install' === $plugin_table->current_action() || 'tgmpa-bulk-update' === $plugin_table->current_action() ) && $plugin_table->process_bulk_actions() ) || $this->do_plugin_install() ) {
				if (isset($thegemThemeOptions)) {
					$thegemThemeOptions->postPageWrapper('install-required-plugins');
				}
				return;
			}
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( admin_url('admin.php?page=thegem-dashboard-welcome' )) . '">' . esc_html__( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ), '</p>';
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function no_items() {
			echo esc_html__( 'No plugins to install, update or activate.', 'tgmpa' ) . ' <a href="' . esc_url( admin_url('admin.php?page=thegem-dashboard-welcome' )) . '"> ' . esc_html__( 'Return to the Dashboard', 'tgmpa' ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}

...
$legend_html = '';
if ( $legend ) {
	foreach ( $datasetValues as $k => $v ) {
		$legend_html .= '<li><span style="background-color:' . $datasetColors[$k] . '"></span>' . $labels[$k] . '</li>';
	}
...
$output = '
<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $options ) . ' '.$styles.'>
	' . $title . '

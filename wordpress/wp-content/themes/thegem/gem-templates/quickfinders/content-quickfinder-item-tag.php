<?php
	...
	if(isset($params['effects_enabled_delay']) && !empty($params['effects_enabled_delay']) && $i === 1) {
		$delay = 'data-ll-item-delay="'.$params['effects_enabled_delay'].'"';
	}
?>
<div id="post-<?php the_ID(); ?>" <?php if($params['effects_enabled']) echo ' data-ll-finish-delay="200" '; ?> <?php post_class(array('quickfinder-item', 'inline-column', $thegem_columns_class, $thegem_quickfinder_effect, 'icon-size-'.$thegem_item_data['icon_size'], 'quickfinder-box-style-'.$params['box_style'], $params['effects_enabled'] ? 'lazy-loading' : '')); ?> <?php echo $delay; ?>>

...
class TheGem_Template_Element_Checkout_Thanks_Billing_Details  extends TheGem_Checkout_Thanks_Template_Element {

	public function __construct() {
	}

	public function get_name() {
		return 'thegem_te_checkout_thanks_billing_details';
	}
	...
	public function shortcode_output($atts, $content = '') {
		...
		), $atts, 'thegem_te_checkout_thanks_billing_details');
		...
		$title_classes = implode(' ', array('woocommerce-billing-details__title', $params['heading_text_style'], $params['heading_font_weight']));
        ...
        <div <?php if (!empty($params['element_id'])): ?>id="<?=esc_attr($params['element_id']); ?>"<?php endif;?>
             class="thegem-te-checkout-thanks-billing-details <?= esc_attr($params['element_class']); ?> <?= esc_attr($uniqid); ?>">
        
            <?php if (!empty($params['heading'])) : ?>
                <<?= $title_tag; ?> class="<?= $title_classes; ?>"><?php esc_html_e( 'Billing address', 'woocommerce' ); ?></<?= $title_tag; ?>>
            <?php endif; ?>
            
            <address class="woocommerce-billing-details">
		...
		<?php
		//Custom Styles
		$customize = '.thegem-te-checkout-thanks-billing-details.'.$uniqid;
		$custom_css = '';
		$resolution = array('desktop', 'tablet', 'mobile');
		
		// Heading Styles
		if (!empty($params['heading_alignment'])) {
			$custom_css .= $customize.' .woocommerce-billing-details__title {text-align: ' . $params['heading_alignment'] . ';}';
		}
		if (!empty($params['heading_letter_spacing'])) {
			$custom_css .= $customize.' .woocommerce-billing-details__title {letter-spacing: ' . $params['heading_letter_spacing'] . 'px;}';
		}
		if (!empty($params['heading_text_transform'])) {
			$custom_css .= $customize.' .woocommerce-billing-details__title {text-transform: ' . $params['heading_text_transform'] . ';}';
		}
		if (!empty($params['heading_text_color'])) {
			$custom_css .= $customize.' .woocommerce-billing-details__title {color: ' . $params['heading_text_color'] . ';}';
		}
		foreach ($resolution as $res) {
			if (!empty($params['heading_spacing_'.$res]) || strcmp($params['heading_spacing_'.$res], '0') === 0) {
				...
				if ($res == 'desktop') {
					$custom_css .= $customize.' .woocommerce-billing-details__title {margin-bottom:'.$result.$unit.' !important;}';
				} else {
					$width = ($res == 'tablet' ? 'max-width: 1023px' : 'max-width: 767px');
					$custom_css .= '@media screen and ('.$width.') {'.$customize.' .woocommerce-billing-details__title {margin-bottom:'.$result.$unit.' !important;}}';
				}
			}
		}
		...
		if (!empty($params['list_color'])) {
			$custom_css .= $customize.' .woocommerce-billing-details {color: ' . $params['list_color'] . ' !important;}';
		}
	...
	public function shortcode_settings() {
		return array(
			...
			'base' => 'thegem_te_checkout_thanks_billing_details',
			'icon' => 'thegem-icon-wpb-ui-element-checkout-billing',
			'category' => __('Purchase Summary Builder', 'thegem'),
			'description' => __('Purchase Summary Billing Details', 'thegem'),
			'params' => array_merge(
...
$templates_elements['thegem_te_checkout_thanks_billing_details'] = new TheGem_Template_Element_Checkout_Thanks_Billing_Details();
$templates_elements['thegem_te_checkout_thanks_billing_details']->init();
